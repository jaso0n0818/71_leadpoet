        ...
        if role_passed:
            ...
            if api_key:
                ...
                if aimode_result["answer"] == "yes":
                    ...
                    result['data']['role_method'] = 'aimode_reject'
                    result['rejection_reason'] = {
                        "stage": "Stage 4: Lead Validation",
                        "check_name": "lead_validation_stage4",
                        "message": f"Role '{role}' rejected by AI Mode: {aimode_result.get('evidence', '')[:200]}",
                        "failed_fields": ["role"],
                        "claimed_role": role,
                        "aimode_evidence": aimode_result.get("evidence", ""),
                    }
                    print(f"   ❌ Role rejected (AI Mode): {aimode_result.get('evidence', '')[:80]}")
                    return result
                else:
